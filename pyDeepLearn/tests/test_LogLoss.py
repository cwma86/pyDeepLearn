...
class TestLogLoss(unittest.TestCase):
    def test_eval1(self):
      ...
      logging.debug("The log loss layer eval() returned \n" +
                   f"{output}"
                  )
    ...
    def test_gradient(self):
      logging.info("Running test_gradient")
      obj_func = LogLoss()
      y = 0.0
      y_hat = 0.2
      output = obj_func.gradient(y, y_hat)
      expected = 1.2499921875488276
      logging.debug("The log loss layer gradient() returned \n" +
                   f"{output}"
                  )
      self.assertTrue(np_array_comp(output, expected))
      logging.info("test_gradiant ran!")
